...
using IGT.PA.Framework.EgmClients.Interfaces;
using IGT.PA.Framework.EgmClients.Interfaces.Enumerations;
using IGT.PA.Framework.EgmClients.Protocols;
using System.IO;
using IGT.PA.Framework.EgmClients.Avp.Rpc;
...
namespace IGT.TestFramework.EGM
{
    ...
    {
        private AvpEgm egm;
        public bool IpConnected = false;
        private static string DefaultIP = "127.0.0.1";

        public bool ConnectIp(string ip = "127.0.0.1")
        {
            try
            {
                    egm = new AvpEgm(ip);
                    egm.FastConnectNoConfig = true;
                    bool connected = egm.Connect();
                    IpConnected = true;
        ...
        {
            get { return ((serialPort != null) && serialPort.IsOpen) || (IpConnected); }
        }
        ...
        public event EventHandler<EGMDisconnectArgs> EGMDisconnected;
        ...
        private void WriteCOM(string[] commands)
        {
            ...
            else
            {
                try
                {
                    ...
                    Array.ForEach(commands, cmd =>
                    {
                        egm.PadPassThruSendCommand((cmd + "\r\n"));
                        //if (InterCommandDelay > 0)
                        //Thread.Sleep(300);
                    });
                }
                ...
                {
                    // The connection has been lost, call EGMDisconnected event handler
                    if (ex is InvalidOperationException || ex is IOException)
                    {
                        EGMDisconnected(this, new EGMDisconnectArgs(ex.ToString()));
                    }
                }
            }
            ...
            else
            {
                ...
                for (int i = 0; i < commands.Length; i++)
                {
                    try
                    {
                        if (i == commands.Length - 1)
                        {
                            lock (this)
                            {
                                waitingForReply = true;
                                response = "";
                                egm.PadPassThruSendCommand((commands[i] + "\r\n"));
                    ...
                    catch (Exception ex)
                    {
                        ...
                        // If the connection has been lost, call EGMDisconnected event handler
                        if (ex is InvalidOperationException || ex is IOException)
                        {
                            EGMDisconnected(this, new EGMDisconnectArgs(ex.ToString()));
                        }
        ...
        private void SerialPortReadThreadMethod()
        {
            ...
            while (IsConnected)
            {
                //don't waste processor time
                Thread.Sleep(1);

                if (!IpConnected && serialPort.IsOpen)
                {
                    try
                    {
                    ...
                    {
                        if (ex is System.InvalidOperationException)     //lost serial port connection
                            break;
                    }
                ...
                {
                    if (!(egm.Api as AvpEgmApi).IsConnected)
                    {
                        EGMDisconnected(this, new EGMDisconnectArgs("EgmApi is not connected"));
                        break;
                    }

                    try
                    {
                        newMessages = egm.PadGetMessages();
                    }
                    catch (Exception ex)
                    {
                        if (ex is InvalidOperationException || ex is IOException)
                    ...
                    if (newMessages.Count > 0)
                    {
                        ...
                        for (int i = 0; i < newMessages.Count; i++)
                            egm.RemoveItemFromPadMessages(i);
                    }
                }
        ...
        private StringBuilder ProcessString(StringBuilder inBuffer)
        {
            if (inBuffer.ToString().EndsWith("\r\n") || inBuffer.ToString().EndsWith("\n\r"))
            {
                if (waitingForReply)
                {
                    //File.WriteAllText(currentDir + "\\debug.txt", inBuffer.ToString());
                    if (inBuffer.ToString().Trim().StartsWith("RX:"))
                    {
                        lock (this)
                        {
